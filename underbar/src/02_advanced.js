...
_.every = function (arr, iteratee) {
  ...
  _.each(arr, function (value) {
    if ((!iteratee && !value) || (iteratee && !iteratee(value))) {
      return (bool = false);
    }
  });
...
_.defaults = function (...argu) {
  // TODO: 여기에 코드를 작성합니다.
  let result = argu[0];

  for (let i = 1; i < argu.length; i++) {
    _.each(argu[i], function (value, key) {
...
_.zip = function (...argu) {
  // TODO: 여기에 코드를 작성합니다.
  let result = [];
  let idx = Math.max(..._.map(argu, (v) => v.length));

  for (let i = 0; i < idx; i++) {
    result.push(_.pluck(argu, i));
  }
  return result;
};
...
_.zipStrict = function (...argu) {
  // TODO: 여기에 코드를 작성합니다.
  let result = [];
  let idx = Math.min(..._.map(argu, (v) => v.length));
...
//                      // 첫 번째 배열에 'e'가 먼저 등장
_.intersection = function (...argu) {
  // TODO: 여기에 코드를 작성합니다.
  let result = [];

  for (let j = 0; j < argu[0].length; j++) {
    let count = 0;

    _.each(argu, (v) => {
      if (_.includes(v, argu[0][j])) {
        count++;
        if (count === argu.length) {
          result.push(argu[0][j]);
        }
      }
    });
  }
...
_.difference = function (...argu) {
  ...
  for (let j = 0; j < argu[0].length; j++) {
    ...
    _.each(argu, (v, i) => {
      if (i !== 0 && !_.includes(v, argu[0][j])) {
        count++;
        if (count === argu.length - 1) {
          result.push(argu[0][j]);
        }
      }
    });
